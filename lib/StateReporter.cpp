...
void state_reporter::StateReporter::try_to_connect() {
    ...
    rc = setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (const void *) &len, sizeof(len));
    if (rc < 0) {
        close(sock);
        return;
    }
...
void state_reporter::StateReporter::send_prepared_messages() {
    while (!outgoing_queue.empty()) {
        ...
        while (already_sent != current_piece.size()) {
            ssize_t rc = send(client_sock, current_piece.data() + already_sent, current_piece.size() - already_sent,
                              MSG_NOSIGNAL);
            if (rc < 0) {
